...
function validator(formSelecter) {
  ...
  for (var input of inputs) {
	...
	if (input.nodeName == 'LABEL') {
		input.addEventListener("toggle", handleValidate);
		input.addEventListener("change", handleValidate);
		input.addEventListener("cuechange", handleValidate);
	}
  ...
  formElement.onsubmit = function (event) {
    ...
    for (input of inputs) {
      ...
      if (!handleValidate({ target: input, test: input })) 
	  	 isValid = false;
    }
    ...
    if (_this.onsubmit) {
      ...
      var arrayInputs = Array.from(enableInputs);
	  
      var formData = Array.from(enableInputs).reduce(function (values, input) {
      ...
      formElement.onsubmit(formData); // standard onsubmit of form
    }
  };
}
